...
//            
//        } else {
//            $konten['rowdata'] = $this->menu_model->getMenu($this->uri->segment(2));
//            $konten['parent'] = $this->uri->segment(2);
//            $konten['page'] = 'menu/submenu';
//            $konten['nav'] = 'mmenu';
...
//        }
    ...
    public function sbm() {
        $konten['rowdata'] = $this->menu_model->getMenu($this->uri->segment(4));
        $konten['parent'] = $this->uri->segment(4);
        $konten['page'] = 'menu/submenu';
        $konten['nav'] = 'mmenu';
        $this->load->view('dashboard', array('data' => $konten));
    }
    ...
    public function getMenu() {
        $menuparent = $this->uri->segment(4);
        $aColumns = array("menu_id", "lang_id", "menu_name", "ordering", "is_active", "parent_id");
        ...
        $sQuery = "
                SELECT
                          menu_id,lang_id,menu_name,ordering,is_active,level_menu,parent_id
                     FROM
                         $sTable WHERE parent_id=$menuparent $sWhere 
                    $sOrder LIMIT $xDisplayLength $nextLimit";

        $rResult = $this->db->query($sQuery);
#echo $this->db->last_query();
        $sQuery = "
		SELECT COUNT(*) as aTot FROM $sTable WHERE parent_id=$menuparent $sWhere 
	";
		...
		FROM   $sTable WHERE parent_id=$menuparent $sWhere 
	";
    ...
    function getForm() {
        ...
        if ($paramid == '') {
            ...
            $konten['rowdata'] = $this->menu_model->ambilMenu($paramid, 1);
        }
        $this->db->flush_cache();
...
//        
    ...
    public function deleteMenu() {
        ...
        $recheckchild = $this->menu_model->ambilDataMenuByParentId($_POST['cid'], '*', '', $_POST['langid']);

        if ($recheckchild) {
            $msg = 'Coud not delete this menu.<br>This menu has active menu child, delete all child first';
            $this->menulist($_GET['langid'], $msg);
        } else {
