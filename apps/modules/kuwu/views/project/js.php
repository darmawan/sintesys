...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('#openform').on('click', function () {
            ...
            $('#form' + $('#tabel').val()).load('project/form');
            setTimeout(function () {
                $('#containerform').show();
                $(".page-loader").fadeOut();
                scrollTo();
...
<?php if ($menuinfo->ubah == 1) { ?>
                                    display = display + ' <button class="btn btn-info waves-effect" data-kode="' + kode + '" data-bind="ubah" data-tabel="project" data-bhs="' + kode + '"><i class="zmdi zmdi-edit"></i></button>';
<?php } ?>
<?php if ($menuinfo->hapus == 1) { ?>
                                    display = display + ' <button class="btn btn-danger waves-effect" data-value="' + kode + '" data-bind="hapus" data-get="project/hapus" data-tabel="project" data-default="' + idnya + '" data-bhs="' + row[1] + '"><i class="zmdi zmdi-delete"></i></button>';
<?php } ?>
<?php if ($menuinfo->tambah == 1) { ?>
                                    display = display + ' <button class="btn bgm-white  waves-effect" data-kode="' + row[0] + '" data-bind="salin" data-get="project/form" data-tabel="project" data-bhs="' + row[6] + '"><i class="zmdi zmdi-copy"></i></button>';
<?php } ?>
                                return display;
