...
<script type="text/javascript">
    ...
    (function ($) {
        ...
        $('#openform').on('click', function () {
             $(".page-loader").fadeIn();
            $('#form' + $('#tabel').val()).load("galeri/form", function (response, status, xhr) {
                if (status == "error") {
        ...
        myTable("#kkTable",
                {
                    ajaxSource: "galeri/sumber/" + $('#tabel').val(),
                    columnDefs: [
                        {targets: [0], width: "45px"},
                        {targets: [2], render: function (data, type, row) {
                                var m = '<img width="120" style="background-color:rgba(153, 153, 153,0.8);" src="<?php echo base_url('publik/rabmag/galeri/thumb'); ?>/' + data + '">';
                                return m;
                            }, width: "180px"},
                        {targets: [3], width: "95px"},
...
<?php if ($menuinfo->ubah == 1) { ?>
                                    display = display + ' <button class="btn btn-info waves-effect" data-kode="' + kode + '" data-bind="ubah" data-tabel="galeri" data-bhs="' + row[1] + '"><i class="zmdi zmdi-edit"></i></button>';
<?php } ?>
<?php if ($menuinfo->hapus == 1) { ?>
                                    display = display + ' <button class="btn btn-danger waves-effect" data-value="' + kode + '" data-bind="hapus" data-get="galeri/hapus" data-tabel="galeri" data-default="' + idnya + '" data-bhs="' + row[2] + '"><i class="zmdi zmdi-delete"></i></button>';
<?php } ?>
                                return display;
