...
<script type="text/javascript">
    ...
    $(function () {
        ...
        myTable("#kkTable",
                {
                    ajaxSource: "menu/sumber/",
                    columnDefs: [
                        {targets: [0], width: "5%"},
                        {targets: [1], width: "30%"},
                        {targets: [2], width: "10%"},
                        {targets: [3], width: "20%"},
                        {targets: [4], sClass: "text-center", width: "10%"},
...
<?php if ($menuinfo->ubah == 1) { ?>
                                    display = display + ' <button class="btn btn-info waves-effect" data-kode="' + kode + '" data-bind="ubah" data-tabel="menu/" data-bhs="' + row[0] + '"><i class="zmdi zmdi-edit"></i></button>';
<?php } ?>
<?php if ($menuinfo->hapus == 1) { ?>
                                    display = display + ' <button class="btn btn-danger waves-effect" data-value="' + kode + '" data-bind="hapus" data-get="menu/hapus" data-tabel="menu" data-default="' + idnya + '" data-bhs="' + row[1] + '"><i class="zmdi zmdi-delete"></i></button>';
<?php } ?>
<?php if ($menuinfo->tambah == 1) { ?>
                                    display = display + ' <button class="btn bgm-white  waves-effect" data-kode="' + kode + '" data-bind="salin" data-get="menu/form" data-tabel="menu" data-bhs="' + row[1] + '"><i class="zmdi zmdi-copy"></i></button>';
<?php } ?>
                                return display;
                            }, sClass: "center p-r-0", sortable: false, width: "15%"},
                    ]
                }
        );
