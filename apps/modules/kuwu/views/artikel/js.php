...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('#openform').on('click', function () {
            ...
            $('#form' + $('#tabel').val()).load('artikel/form');
            setTimeout(function () {
                $('#containerform').show();
                 $(".page-loader").fadeOut();
                scrollTo();
        ...
        myTable("#kkTable",
                {
                    ajaxSource: "artikel/sumber/" + $('#tabel').val(),
                    columnDefs: [
...
<?php if ($menuinfo->ubah == 1) { ?>
                                    display = display + ' <button class="btn btn-info waves-effect" data-kode="' + kode + '" data-bind="ubah" data-tabel="artikel" data-bhs="' + row[1] + '"><i class="zmdi zmdi-edit"></i></button>';
<?php } ?>
<?php if ($menuinfo->hapus == 1) { ?>
                                    display = display + ' <button class="btn btn-danger waves-effect" data-value="' + kode + '" data-bind="hapus" data-get="artikel/hapus" data-tabel="artikel" data-default="' + idnya + '" data-bhs="' + row[1] + '"><i class="zmdi zmdi-delete"></i></button>';
<?php } ?>
<?php if ($menuinfo->tambah == 1) { ?>
                                    var lang = (row[1] === '1') ? 2 : 1;
                                    if (parseInt(row[6]) === 1 && parseInt(row[1]) === 1) {
                                        display = display + ' <button class="btn bgm-white  waves-effect" data-kode="' + row[0] + '" data-bind="salin" data-get="artikel/form" data-tabel="artikel" data-bhs="' + row[1] + '"><i class="zmdi zmdi-copy"></i></button>';
                                    } else {
                                        var display = display;
                                    }
